...
use crate::logical_plan::utils::{
    from_qualified_name, map_data_type,
};
use crate::mdl;
...
impl ModelPlanNode {
    pub fn new(
        ...
        let fields = model
            ...
            .map(|column| {
                if column.is_calculated {
                    ...
                    let expr_plan = Expr::Column(Column::from_name(column.name()));
        ...
        let start = directed_graph.node_indices().next().unwrap();
        ...
        for x in directed_graph.neighbors_directed(start, Outgoing) {
            let link = directed_graph.find_edge(start, x).unwrap();
            let link = directed_graph.edge_weight(link).unwrap();
            let target = directed_graph.node_weight(x).unwrap();
...
fn get_column_exp(
    ...
    let expr = create_wren_expr_for_model(&column.name, model, analyzed_wren_mdl);
    expr.alias(column.name.clone())
}
    ...
    pub(crate) fn plan(&mut self, rule: ModelGenerationRule) -> Option<LogicalPlan> {
        match self {
            RelationChain::Chain(plan, _, condition, ref mut next) => {
                ...
                left.schema().fields().iter()
                    .map(|field| OrdExpr::new(col(field.name())))
                    .for_each(|c| {
                        required_exprs.insert(c.clone());
                    });
                dbg!(left.schema());

                let mut required_field: Vec<Expr> =
                    required_exprs.iter().map(|expr| expr.expr.clone()).collect();
                dbg!(required_field.clone());
                Some(
